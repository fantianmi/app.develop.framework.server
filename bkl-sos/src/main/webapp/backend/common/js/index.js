...
function addArcType(){
	...
	var params={reid:reid, sortrank:sortrank, defaultname:defaultname, description:description, keywords:keywords, issend:issend, corrank:corrank, siteurl:siteurl, sitepath:sitepath, channel:channel};
	jQuery.post(url,params,function(data){
		if(data) {
			if(data.ret == 0) {
				...
				$("#folderContent"+reid+"").append("<div class='tree-folder' style='display:block'  id='folder"+menuId+"'><div class='tree-folder-header'><i class='icon-plus' onclick='loadChildData("+menuId+")' id='iconCheck"+menuId+"'></i><div class='tree-folder-name'>"+menuName+"</div>&nbsp;&nbsp;[<a href='javascript:showOperationPanel("+menuId+");'>增加下级目录</a>]&nbsp;&nbsp;[<a href='javascript:deleteArcType("+menuId+");'>删除</a>]</div>");
				alert("添加成功！");
			}
		}
	});
...
/*删除栏目*/
function deleteArcType(id){
	if(confirm("确认删除栏目"+id+"？")){
		var url="/arctype/deleteArcType?random="+Math.round(Math.random()*100);
		var params={id:id};
		jQuery.post(url,params,function(data){
			if(data) {
				if(data.ret == 0) {
